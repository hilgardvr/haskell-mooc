...
data State = Start | EggsAdded | FlourAdded | SugarAdded | ReadyForMix | Mixed | Error | Finished
  deriving (Eq,Show)
...
step state event = case state of
    ...
    FlourAdded -> 
        case event of
            AddSugar -> ReadyForMix
            _ -> Error
    SugarAdded ->
        case event of
            AddFlour -> ReadyForMix
            _ -> Error
    ReadyForMix -> 
        case event of 
            Mix -> Mixed
...
average (a :| as) = foldr (+) a as / (fromIntegral (length as) + 1)
...
reverseNonEmpty (a :| as) =
    let reversed = reverse as ++ [a]
...
passwordAllowed s (MinimumLength m) = length s >= m

passwordAllowed [] (ContainsSome cs) = False
passwordAllowed (x:xs) (ContainsSome cs) = if elem x cs then True else passwordAllowed xs (ContainsSome cs)

passwordAllowed [] (DoesNotContain dn) = True
passwordAllowed (x:xs) (DoesNotContain dn) = if elem x dn then False else passwordAllowed xs (DoesNotContain dn)
