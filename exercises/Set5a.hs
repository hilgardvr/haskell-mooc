...
study (NthYear i) = if i < 7 then NthYear (i + 1) else Graduated
...
toNat z = 
    let
        helper i = case compare i 0 of 
            LT -> Zero
            EQ -> Zero
            GT -> PlusOne (helper (i - 1))
    in 
        if z < 0 then Nothing else Just (helper z)
...
fromBin bin =
    let
        h :: Bin -> Int -> Int
        h End val = 0
        h (O b) val = 0 + h b (val*2)
        h (I b) val = val + h b (val*2)
    in
        h bin 1
