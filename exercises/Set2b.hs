...
oddFactorial i =
    let
        oddFactorialHelper j acc
            | j <= 1 = acc
            | odd j = oddFactorialHelper (j - 2) (j * acc)
            | otherwise = oddFactorialHelper (j - 1) acc
    in
        oddFactorialHelper i 1
...
countdown i = 
    let
        builder i s = 
            if i <= 1 
            then "Ready! " ++ s ++ "1... Liftoff!"
            else builder (i - 1) (s ++ show i ++ "... ")
    in
        builder i ""
