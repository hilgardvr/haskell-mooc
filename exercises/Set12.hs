...
count val foldable = foldr (\e acc -> if e == val then acc+1 else acc) 0 foldable
...
inBoth a b = foldr (\e acc -> if elem e b then e:acc else acc) [] a
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode a b tl) = f a (f b (foldr f acc tl))
