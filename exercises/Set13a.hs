...
checkNumber (f, s) = 
    if any isDigit f || any isDigit s
    then Nothing
    else Just (f, s)
...
selectSum xs is = 
    mapM (\e -> safeIndex xs e) is >>= return . sum
    where
...
countAndLog f (x:xs) = 
    if f x
    then countAndLog f xs >>= (\l -> Logger [show x] (1+l))
...
balance accountName = BankOp helper
    where 
        ...
        helper (Bank b) = case Map.lookup accountName b of 
            Nothing -> (0, (Bank b))
            Just v -> (v, (Bank b))
